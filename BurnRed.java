 ...
 */
@Autonomous(name="Red Burn", group="Red")
@Disabled
public class BurnRed extends LinearOpMode {
...
*/
       ...
       // encoderDrive(.5, 24)
        ...
        stopMovement();
        sleep(100);
        while (opticalSensor.getLightDetected() < WHITE_THRESHOLD || !isStopRequested()){
            encoderStrafe(.5, -.5, 1);
        }
        stopMovement();
        sleep(100);
   ...
   */
    ...
    public void encoderDrive(double power, double inches) {
        ...
        while (leftFront.isBusy() && rightFront.isBusy() && leftBack.isBusy() && rightBack.isBusy() || !isStopRequested()) {
        }
    ...
    public void gyroTurn(double power, int angle){


        int heading = gyro.getHeading();

        if (heading > 180) {
            heading = heading - 360; //POTENTIAL AREA OF PROBLEM
        }

        int error = heading - angle;

        encoderTurn(power, -error);
